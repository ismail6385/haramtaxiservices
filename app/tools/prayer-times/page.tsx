            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
                        Prayer Times inside <span className="text-slate-600">Saudi Arabia</span>
                    </h1>
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-start">
                    ...
                    <div className="bg-white rounded-2xl p-8 shadow-sm">
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            {CITIES.map((city) => (
                                <Link key={city.slug} href={`/tools/prayer-times/${city.slug}`}>
                                    <div className="group border border-gray-100 rounded-xl p-4 hover:border-slate-500 hover:shadow-md transition-all cursor-pointer flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 bg-slate-50 rounded-full flex items-center justify-center group-hover:bg-slate-500 transition-colors">
                                                <MapPin className="w-5 h-5 text-slate-600 group-hover:text-white" />
                                            </div>
                                            <span className="font-semibold text-gray-800">{city.name}</span>
                                        </div>
                                        <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-slate-500 transform group-hover:translate-x-1 transition-all" />
                                    </div>
                                </Link>
                            ))}
                        </div>
                        ...
                        <div className="mt-8 bg-slate-50 rounded-xl p-6">
                            <h3 className="font-bold text-slate-900 mb-2">Why Accurate Timings Matter?</h3>
