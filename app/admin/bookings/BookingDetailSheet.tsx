...
export default function BookingDetailSheet({
    booking, open, onClose, onStatusChange, onDriverAssign, onPriceUpdate, onNoteSave, updatingId,
}: Props) {
    ...
    const waMessage = encodeURIComponent(`Hello ${booking.customer_name}, regarding your booking from ${booking.pickup_location} to ${booking.destination} on ${booking.pickup_date} at ${booking.pickup_time}. `)
                ...
                <SheetContent className="w-full sm:max-w-[480px] bg-neutral-950 border-neutral-800 text-white p-0 flex flex-col overflow-hidden">
                    ...
                    <div className="p-5 border-b border-neutral-800 bg-neutral-900 shrink-0">
                        <SheetHeader className="text-left space-y-0.5 mb-4">
                            <SheetTitle className="text-lg font-bold text-white">Booking Details</SheetTitle>
                            <SheetDescription className="text-[11px] text-neutral-600 font-mono">#{booking.id.toUpperCase()}</SheetDescription>
                        </SheetHeader>
                        ...
                        <div className="flex items-center justify-between gap-3">
                            ...
                            <Select value={booking.status} onValueChange={(val) => onStatusChange(booking.id, val as BookingStatus)} disabled={isUpdating}>
                                <SelectTrigger className="h-8 w-[145px] bg-neutral-800 border-neutral-600 text-xs">
                                    {isUpdating ? <Loader2 className="w-3 h-3 animate-spin text-neutral-400" /> : <SelectValue />}
                                </SelectTrigger>
                        ...
                        <div className="flex gap-2 mt-3">
                            <a href={whatsappLink} target="_blank" rel="noreferrer"
                                className="flex-1 flex items-center justify-center gap-1.5 h-9 rounded-lg bg-green-500/10 hover:bg-green-500/20 text-green-400 text-xs font-medium border border-green-500/20 transition-colors">
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className="flex items-center justify-center gap-1 h-9 px-2.5 rounded-lg bg-green-500/10 hover:bg-green-500/20 text-green-400 text-xs font-medium border border-green-500/20 transition-colors">
                                    ...
                                    {WA_TEMPLATES(booking).map(t => (
                                        <DropdownMenuItem key={t.label} asChild>
                                            <a href={`https://wa.me/${phone}?text=${encodeURIComponent(t.message)}`} target="_blank" rel="noreferrer"
                                                className="text-sm text-white cursor-pointer hover:bg-neutral-700">
                                                {t.label}
                                            </a>
                                        </DropdownMenuItem>
                                    ))}
                            ...
                            <a href={mailtoLink}
                                className="flex-1 flex items-center justify-center gap-1.5 h-9 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 text-xs font-medium border border-blue-500/20 transition-colors">
                                <Mail className="w-3.5 h-3.5" /> Email
                            </a>
                        </div>

                        {/* Quick actions row 2 — documents */}
                        <div className="flex gap-2 mt-2">
                            <button onClick={() => setQuoteOpen(true)}
                                className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-400 text-xs font-medium border border-yellow-500/20 transition-colors">
                                <FileText className="w-3.5 h-3.5" /> Quote
                            </button>
                            <button onClick={() => setInvoiceOpen(true)}
                                className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 text-xs font-medium border border-purple-500/20 transition-colors">
                                <FileText className="w-3.5 h-3.5" /> Invoice
                            </button>
                            <button onClick={() => setReceiptOpen(true)}
                                className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-xs font-medium border border-emerald-500/20 transition-colors">
                        ...
                        <div className="p-5 space-y-6">
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailRow icon={Users} label="Full Name">{booking.customer_name}</DetailRow>
                                    <DetailRow icon={MessageCircle} label="Phone">
                                        <a href={whatsappLink} target="_blank" rel="noreferrer" className="text-green-400 hover:text-green-300 transition-colors">{booking.customer_phone}</a>
                                    </DetailRow>
                                    <DetailRow icon={Mail} label="Email">
                                        <a href={`mailto:${booking.customer_email}`} className="text-blue-400 hover:text-blue-300 transition-colors break-all">{booking.customer_email}</a>
                                    </DetailRow>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailRow icon={MapPin} label="Pickup Location">{booking.pickup_location}</DetailRow>
                                    <DetailRow icon={MapPin} label="Destination">{booking.destination}</DetailRow>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {driverEdit ? (
                                                <div className="flex items-center gap-2">
                                                    <Input autoFocus placeholder="Driver name or ID" value={driverInput}
                                                    ...
                                                    <button onClick={() => setDriverEdit(false)} className="text-neutral-500 hover:text-neutral-300 transition-colors"><X className="w-4 h-4" /></button>
                                                </div>
                                            ) : booking.driver_assigned ? (
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm font-medium text-green-400">{booking.driver_assigned}</span>
                                                    <button onClick={() => { setDriverInput(booking.driver_assigned || ''); setDriverEdit(true) }} className="text-neutral-600 hover:text-neutral-300 transition-colors"><Pencil className="w-3 h-3" /></button>
                                                </div>
                                            ) : (
                                                <button onClick={() => setDriverEdit(true)} className="text-sm text-brand-gold hover:text-yellow-300 transition-colors flex items-center gap-1">
                                            ...
                                            {priceEdit ? (
                                                <div className="flex items-center gap-2">
                                                    <Input autoFocus type="number" placeholder="0" value={priceInput}
                                                        ...
                                                        className="h-8 text-sm bg-neutral-800 border-neutral-600 text-white" />
                                                    <button onClick={handlePriceSave} disabled={savingPrice} className="text-green-400 hover:text-green-300 transition-colors">
                                                        {savingPrice ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                                                    </button>
                                                    <button onClick={() => setPriceEdit(false)} className="text-neutral-500 hover:text-neutral-300 transition-colors"><X className="w-4 h-4" /></button>
                                                </div>
                                            ) : booking.total_price ? (
                                                <div className="flex items-center gap-2">
                                                    <span className="text-base font-bold text-green-400">SAR {booking.total_price.toLocaleString()}</span>
                                                    <button onClick={() => { setPriceInput(String(booking.total_price || '')); setPriceEdit(true) }} className="text-neutral-600 hover:text-neutral-300 transition-colors"><Pencil className="w-3 h-3" /></button>
                                                </div>
                                            ) : (
                                                <button onClick={() => setPriceEdit(true)} className="text-sm text-brand-gold hover:text-yellow-300 transition-colors flex items-center gap-1">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-[11px] text-neutral-500 mb-1">Special Requests</p>
                                            <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-3 text-sm text-neutral-300 whitespace-pre-wrap min-h-[40px] leading-relaxed">
                                                {booking.special_requests || <span className="text-neutral-600">None</span>}
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            {noteEdit ? (
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <Button onClick={handleNoteSave} disabled={savingNote} size="sm" className="bg-brand-gold text-brand-navy hover:bg-yellow-400 h-7 text-xs font-bold">
                                                ...
                                                <div className="flex items-start gap-2">
                                                    <div className="flex-1 bg-neutral-900 border border-neutral-800 rounded-lg p-3 text-sm text-neutral-300 whitespace-pre-wrap leading-relaxed">{booking.admin_notes}</div>
                                                    <button onClick={() => { setNoteInput(booking.admin_notes || ''); setNoteEdit(true) }} className="text-neutral-600 hover:text-neutral-300 mt-2 transition-colors shrink-0"><Pencil className="w-3 h-3" /></button>
                                                </div>
                                            ) : (
                                                <button onClick={() => { setNoteInput(''); setNoteEdit(true) }} className="text-sm text-neutral-500 hover:text-neutral-300 transition-colors flex items-center gap-1">
