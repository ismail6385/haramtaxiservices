    ...
    return (
        <Link href={href} className="block group h-full">
            <div className="h-full bg-neutral-900 rounded-none overflow-hidden transition-all duration-300 border border-white/10 hover:border-amber-500/50 hover:bg-neutral-800 flex flex-col group relative">
                ...
                {/* Decorative Corner */}
                <div className="absolute top-0 right-0 w-12 h-12 bg-yellow-500/10 -mr-6 -mt-6 rotate-45 group-hover:bg-yellow-500 transition-colors duration-500"></div>
                ...
                <div className="relative h-64 w-full bg-neutral-800 overflow-hidden">
                    ...
                    <div className="absolute bottom-4 left-6">
                        <h3 className="text-2xl font-bold text-white font-serif tracking-wide group-hover:text-yellow-500 transition-colors">
                            {name}
                        </h3>
                    </div>
                </div>
                ...
                <div className="p-6 flex-grow flex flex-col relative">
                    ...
                    <ul className="space-y-3 mb-8 flex-grow">
                        {features.map((feature, index) => (
                            <li key={index} className="text-neutral-400 text-sm flex items-center group-hover:text-neutral-300 transition-colors">
                                <span className="w-1.5 h-1.5 bg-yellow-500 rounded-full mr-3 opacity-50 group-hover:opacity-100 transition-opacity"></span>
                                {feature}
                            </li>
                        ))}
                    </ul>
                    ...
                    {/* CTA Button */}
                    <Button className="w-full bg-transparent hover:bg-yellow-600 text-white border border-white/20 hover:border-amber-600 font-serif tracking-widest text-xs uppercase h-12 rounded-none transition-all group-hover:shadow-[0_0_20px_rgba(217,119,6,0.2)]">
                        View {name} Details
                    </Button>
                </div>
            </div>
